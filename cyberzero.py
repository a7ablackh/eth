...
parser.add_argument('-threads', dest = "THREAD_NUMBER", help="Number of threads (recommended <= 3)", default=50, type=int, required=False)
...
if proxy_use.lower() == 'yes':
    with open(os.path.join(os.path.dirname(__file__), proxy_file), 'r') as file:
        w3 = Web3(HTTPProvider('https://ethereum.publicnode.com',request_kwargs={"proxies":file.read()}))
main()
